...
class SinglePopEncoder(nn.Module):
    ...
    def __init__(self, input_size, params, *args, **kwargs):
        ...
        self.syn_pop_encoder = [None] * self.input_size
    ...
    def forward(self, input_):
        ...
        for i in range(self.input_size):
            ...
            spk_pop_encoded_feature_i, self.syn_pop_encoder[i], self.mem_pop_encoder[i] = self.spk_pop_encoder_list[i](
                pop_encoded_feature_i, self.syn_pop_encoder[i], self.mem_pop_encoder[i])
            spk_pop_encoded_signal.append(spk_pop_encoded_feature_i)

        spk_pop_encoded_signal_rec = torch.cat(spk_pop_encoded_signal, 1)

        return spk_pop_encoded_signal_rec

    def init_pop_encoder(self):
        for i in range(self.input_size):
            self.syn_pop_encoder[i], self.mem_pop_encoder[i] = self.spk_pop_encoder_list[i].init_synaptic()
